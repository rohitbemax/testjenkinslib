def call(String chartVersion) {
    ...
    sh """
        ...
        gcloud pubsub topics publish ${TOPIC_ID} --attribute event_type=CONTINUOUS_DEPLOYMENT_REQUEST,correlation_id=\$CORRELATION_ID --message="{\"component_name\": \"one-platform-solution-worker\",\"helm_chart_name\": \"one-platform-solution-worker\",\"helm_chart_version\": \"$chartVersion\", \"docker_image_tag\": \"\$VERSION-${env.GIT_SHORT_HASH}\" }"
